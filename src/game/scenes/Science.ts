...
import { Sciencequestions, ScienceQuestionsHard } from '../../question'; 
interface Question {
    question: string;
    answers: string[];
...
export class Science extends Scene {
    ...
    create(data: { level: string }) {
        ...
        if (data.level === 'Hard') {
            this.questions = ScienceQuestionsHard; 
            this.startTimer();
        } else {
    ...
    resetTimer() {
        this.timer = 30; 
        this.timerText?.setText(`Time: ${this.timer}`);
        this.timerEvent?.reset({ delay: 1000, callback: this.onTimerTick, callbackScope: this, loop: true }); 
    }
